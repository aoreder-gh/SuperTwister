# ================================================================================================
# Super Twister 3001
# help_wizard.py --- Interactive setup wizard for string type selection and step-by-step guidance
...
class HelpDialog(BaseDialog):
    ...
    def __init__(self, root):
        ...
        if state.wizard_active and state.wizard_bow_type:
            self.resume_wizard()
        else:
            self.show_bow_selection()
    ...
    def load_config(self):
        path = os.path.join("configs", "help_wizard.json")
        try:
            with open(path, "r") as f:
                return json.load(f)
        except Exception as e:
            messagebox.showerror("Error", f"Wizard configuration error:\n{e}")
            return {}
    ...
    def resume_wizard(self):
        self.bow_type = state.wizard_bow_type
        self.steps = self.config_data[self.bow_type]["steps"]
    ...
    def reset_wizard(self):
        ...
        # reset_btn is hidden in show_bow_selection, so no need to disable it here
        self.reset_btn.place_forget()
    ...
    def _on_done_changed(self, *args):
        if not self.bow_type:
            return

        key = f"{self.bow_type}_{self.current_step}"
    ...
    def show_bow_selection(self):
        self.clear_page()

        t = language.texts[state.language]

        self.title_label.config(text=t["chooseStringType"])
    ...
    def start_wizard(self, bow_key):
        ...
        state.wizard_bow_type = bow_key
        state.wizard_step_index = 0
        self.bow_type = bow_key
    ...
    def show_step(self):
        ...
        info = f"RPM: {step['rpm']}     Turns: {turns}     Direction: {'CW ↻' if step['direction'] else 'CCW ↺'}"
        ...
        # Apply system parameters for this step
        state.target_rpm = step["rpm"]
        state.remaining_turns = step["turns"]
    ...
    def next_step(self):
        ...
        if not self.done_var.get():
            t = language.texts[state.language]
            messagebox.showwarning(t["info"], t["wizard_step"])
            return
